...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy",
        required=True,
        help="Frozen DEFT OD AOI policy containing the cumulative synthetic cap.",
    )
    parser.add_argument(
...
def synthetic_fraction_cap(path: str | Path) -> float:
    policy = yaml.safe_load(Path(path).expanduser().resolve().read_text(encoding="utf-8"))
    try:
        value = float(policy["synthesis"]["cumulative_fraction_of_real_defects"])
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(
            "policy lacks synthesis.cumulative_fraction_of_real_defects"
        ) from exc
    if not 0.0 <= value <= 1.0:
        raise ValueError("synthetic cumulative fraction must be between 0 and 1")
    return value
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for record in base_unique.values():
        ...
        base_kind_counts[kind] = base_kind_counts.get(kind, 0) + 1
    fraction = synthetic_fraction_cap(args.policy)
    synthetic_limit = int(base_kind_counts.get("real_defect", 0) * fraction)
    retained_synthetic = sum(
        record.get("kind") == "synthetic_defect" for record in previous_records
    )
    synthetic_room = max(0, synthetic_limit - retained_synthetic)
    synthetic_candidates: dict[str, dict[str, Any]] = {}
    for record in requested_synthetic_records:
        key = source_key(record["source_path"])
        if key in base_unique:
            if (
                base_unique[key].get("boxes") != record.get("boxes")
                or base_unique[key].get("kind") != record.get("kind")
            ):
                raise ValueError(f"conflicting synthetic source image: {key}")
            continue
    ...
    report = {
        ...
        "synthetic_admission": {
            "fraction_of_real_defects": fraction,
            "cumulative_limit": synthetic_limit,
            "retained_previous": retained_synthetic,
