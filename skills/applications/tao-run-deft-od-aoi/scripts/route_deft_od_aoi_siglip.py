...
def route(args: argparse.Namespace) -> dict[str, Any]:
    ...
    query_manifest = _read_json(args.query_manifest)
    observed_queries = {
        str(key): int(value) for key, value in queries["branch"].value_counts().items()
    }
    if (query_manifest.get("status") != "COMPLETE"
            or int(query_manifest.get("iteration", -1)) != int(args.iteration)
            or query_manifest.get("query_counts") != observed_queries):
        raise ValueError("query manifest is stale or inconsistent with query embeddings")
    source = _index_coco(
    ...
    output_paths = {
        "mined_manifest": output / "mined_manifest.json",
        "synthetic_plan": output / "synthetic_plan.json",
        "routing_report": output / "routing_report.json",
        "defect_ledger": output / "defect_ledger.json",
        "clean_ledger": output / "clean_ledger.json",
        "admission_index": output / "admission_index.npy",
    }
    _atomic_json(output_paths["mined_manifest"], manifest)
    _atomic_json(output_paths["synthetic_plan"], dict(sorted(synthetic_counts.items())))
    _atomic_json(output_paths["routing_report"], report)
    _atomic_json(output_paths["defect_ledger"], sorted(used_defect))
    _atomic_json(output_paths["clean_ledger"], sorted(used_clean))
    admission.save(output_paths["admission_index"])
    input_values = {
    ...
    branch_counts = Counter(str(record.get("branch")) for record in manifest)
    expected = {
        "strict_fn_real": len(selected_real),
        "near_miss_real": len(selected_near),
        "background_clean": len(selected_clean),
    }
    if any(branch_counts.get(branch, 0) != count for branch, count in expected.items()):
        raise ValueError("routing report selected counts do not match mined manifest")
    preview = {
        "schema_version": 1,
        "status": "COMPLETE",
        "iteration": int(args.iteration),
        "controller": {
            "strict": "adaptive_per_pocket",
            "adaptive_conversion_prior": float(routing_policy["adaptive_conversion_prior"]),
            "real_mine_factor_min": int(routing_policy["real_mine_factor_min"]),
            "real_mine_factor_max": int(routing_policy["real_mine_factor_max"]),
            "near_miss_real_factor": int(routing_policy["near_miss_real_factor"]),
            "near_miss_real_cap_per_pocket": int(
                routing_policy["near_miss_real_cap_per_pocket"]
            ),
            "clean_factor": int(routing_policy["clean_factor"]),
            "clean_cumulative_cap_per_real": float(
                routing_policy["clean_cumulative_cap_per_real"]
            ),
        },
        "counts": {
            ...
            "defect_ledger": len(used_defect),
            "clean_ledger": len(used_clean),
            "admission": report["admission"],
        },
        "inputs": {
            name: _file_record(Path(raw))
            for name, raw in input_values.items()
            if raw
        },
        "outputs": {name: _file_record(path) for name, path in output_paths.items()},
    }
    _atomic_json(output / "admission_preview.json", preview)
    return report
