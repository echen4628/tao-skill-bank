...
The miner emits absolute filepaths. This script selects the matching COCO image
records by basename, copies each image into one flat training directory,
renumbers image/annotation ids, preserves the frozen category ids, and writes a
classmap in category-id order. Re-running replaces the JSON and classmap rather
than appending, so the stage is idempotent.
"""
...
def _category_contract(coco: dict) -> tuple[list[dict], list[int], list[str]]:
    ...
    if any(not name for name in names):
        raise ValueError("source COCO category names must be non-empty")
...
def stage(args: argparse.Namespace) -> dict:
    ...
    selected_paths = [Path(str(value)).expanduser().resolve() for value in frame[column]]
    selected_paths = list(dict.fromkeys(selected_paths))
    if not selected_paths:
        raise ValueError("mined parquet selects zero images")

    source = json.loads(source_path.read_text(encoding="utf-8"))
    categories, category_ids, class_names = _category_contract(source)
    ...
    if duplicate_basenames:
        raise ValueError(
            "source COCO has duplicate image basenames, which cannot be staged into one "
            f"flat directory: {sorted(duplicate_basenames)[:8]}"
        )
    ...
    for source_file in selected_paths:
        ...
        staged_images.append(
            {**source_image, "id": image_id, "file_name": source_file.name}
        )
...
def main() -> int:
    try:
        ...
        if rate < args.min_success_rate:
            raise ValueError(
                f"staging success rate {rate:.1%} is below {args.min_success_rate:.1%}"
            )
        print(
