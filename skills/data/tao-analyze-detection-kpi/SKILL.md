...
| `kpi.conf_threshold` | `0.5` | Predictions below this are dropped. **Use `0.0`** (the bundled template and the reference pipeline both do) so AP is computed over the full PR curve rather than a pre-filtered slice. |
...
```bash
SPEC=/absolute/path/to/kpi_analyze.yaml
