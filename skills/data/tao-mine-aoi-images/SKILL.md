...
Run after any label/similarity retention filter, so the input represents the
actual candidates eligible for training:
...
filtering cannot manufacture variance when every iteration retrieves the same
weak neighborhood.
