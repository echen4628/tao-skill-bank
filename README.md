...
That's it — no `git clone`, no `pip install`. The TAO Skill Bank plugin bundles all 69 skills (every model, data, platform, and application). The plugin's [`SessionStart`](hooks/session_start.sh) hook loads the [`AGENTS.md`](AGENTS.md) identity at the start of every session.
